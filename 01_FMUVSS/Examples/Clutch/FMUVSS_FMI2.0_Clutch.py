...
config = {
    'simulation': {
        ...
        'step_size': 1e-4,
        'initial_mode': 'ClutchUncoupled'
    },
    'modes': {
        'ClutchUncoupled': {
            'fmu_path': './model/ClutchUncoupledFMI2.0.fmu',
            'monitored_vars': ['currentTime'],
            'outputs': ['w1', 'f1'],
            'stop_condition': lambda vars: vars['currentTime'] >= 5 and vars['currentTime'] < 7,
            'transition_mapping': {'ClutchCoupled': {'w1': 'w1', 'f1': 'f1'}},
            'next_mode': 'ClutchCoupled'
        },
        'ClutchCoupled': {
            'fmu_path': './model/ClutchCoupledFMI2.0.fmu',
            'monitored_vars': ['currentTime'],
            'outputs': ['w1', 'f1'],
            'stop_condition': lambda vars: vars['currentTime'] < 5 or vars['currentTime'] >= 7,
            'transition_mapping': {'ClutchUncoupled': {'w1': 'w1', 'f1': 'f1'}},
            'next_mode': 'ClutchUncoupled'
        }
    },
    'plot': {
        'x': 'time',
        'y': 'f1',
        'title': 'Opposite Torque of Clutch',
        'xlabel': 'Time',
        'ylabel': 'Opposite Torque',
        'figsize': (6, 6)
    }
}
